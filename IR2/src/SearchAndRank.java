...
public class SearchAndRank {
	...
	public List<Document> search(List<String> queryWords, int taskNum) throws IOException, ParseException {
	    ...
	    ScoreDoc[] hits = isearcher.search(booleanQuery.build(),10).scoreDocs;
	...
	public static void main(String[] args) throws IOException, ParseException {
		if (args.length > 0) {
			...
			// just one query example, for task number 12
			List<String> queryWords;
			queryWords = new LinkedList<String>();
			queryWords.add("comparison");
			queryWords.add("spam");
			queryWords.add("filtering");
			queryWords.add("methods");
			
			// search results
			List<Document> results = engine.search(queryWords,12);
